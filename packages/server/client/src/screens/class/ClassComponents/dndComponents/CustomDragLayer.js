...
const CustomDragLayer = (props) => {
    ...
    function renderItem() {
        return (
            <div
                style={{ boxShadow: item.type === Itemtypes.TASK ? '0px 0px 5px 0px rgba(0,0,0,0.75)' : null }}
                className={item.type === Itemtypes.TASK ? 'task-container' : 'column-container'}
                dangerouslySetInnerHTML={{ __html: draggingContent.innerHTML }}
            />
        );
    }
    ...
    return (
        <div style={layerStyles}>
            <div style={getItemStyles(item, currentOffset)}>{renderItem()}</div>
        </div>
    );
};
