...
import { getProfileImage } from '../../../common/utility-functions';
...
class Task extends Component {
    ...
    render() {
        ...
        return (
            <div id={`id_task_${task.id}`} className="task-container" onClick={this.onClick}>
                <div className="pc-task-flag">
                    ...
                    <div className="pc-deadline-view">
                      {task.Deadline && <p className={task.Deadline < new Date() ? 'expire' : 'no-expire'}>{moment(task.Deadline).format('YYYY-MM-DD')}</p>}
                    </div>
                </div>
                <p className="pc-task-name">{task.name}</p>
                <div className="task-footer">
                    <div className="attached-details">
                      {task.description && <div className="pc-item"><CgDetailsMore size="1em" className="pc-icon" /></div>}
                      {comments > 0 && <div className="pc-item"><FaRegCommentAlt size="0.9em" className="pc-icon" /><span className="pc-count">{comments}</span></div>}
                      {attachments > 0 &&<div className="pc-item"><TiAttachmentOutline size="1em" className="pc-icon" /><span className="pc-count">{attachments}</span></div>}
                    </div>
                    <div className="involved-people">
                      {
                        task.participants.map(o => (
                          <figure key={`p-${o.id}`} className="people-profile-pic">
                            <img src={getProfileImage(o.userDetail)} alt="people" />
                          </figure>
                        ))
                      }
                    </div>
