...
class Profile extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({ apis });
    updateNav({ schema: 'sideNav', data: { name: 'Profile' } });
  }

  togglePopovers = (type) => {
    const { skillsEditPopover } = this.state;
    if (type === 'skills') this.setState({ skillsEditPopover: !skillsEditPopover });
  }

  render() {
    const { account, database, deleteDatabaseSchema, updateDatabaseSchema, addDatabaseSchema } = this.props;
    const { apis } = this.state;
    if (!account.user) return <div />;
    const user = database.User.byId[account.user.id];
    const userDetail = Object.values(database.UserDetail.byId).find(obj => obj.userId === user.id) || {};
