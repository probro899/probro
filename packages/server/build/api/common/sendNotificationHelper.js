...
const dataPropvider = (table, id) => {
  return _cache2.default.get(table).find(b => b.id === id);
...
const findEmailBody = async record => {
  ...
  return {
    html: htmlStringValue.boardNotificationHtml(body, 'Please follow the link to see the changes', 'https://properclass.com'),
    subject: body
  };
};

exports.default = async (context, record) => {
  ...
  try {
    ...
    (0, _sendNotification2.default)(context, emailObj, notificanObj, sessions);
  } catch (e) {
