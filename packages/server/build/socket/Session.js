'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _v = require('uuid/v4');

var _v2 = _interopRequireDefault(_v);

var _ws = require('ws');

var _ws2 = _interopRequireDefault(_ws);

var _proSocketCommon = require('@probro/common/proSocket-common');

var _scoping = require('./scoping');

var _Channel = require('./Channel');

var _Channel2 = _interopRequireDefault(_Channel);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class Session {
  constructor(req, params, ws) {
    this.id = (0, _v2.default)();

    this.req = req;
    this.params = params;

    this.values = {};
    this.cleanUps = {};
    this.scopes = {};
    this.closeListeners = [];
    this.subscriptions = [];

    this.scopes = {};

    this.proxy = null;
    this.ws = ws;

    ws.on('close', () => {
      // Remove all subscriptions
      console.log('onclose method called');
      this.subscriptions.forEach(channelId => _Channel2.default.unsubscribe(channelId, this));

      // Trigger all the close listeners
      this.closeListeners.forEach(c => c());

      // Perform all cleanups
      Object.keys(this.cleanUps).forEach(k => {
        this.cleanUps[k]();
      });
    });
  }

  clearProxy(scopeId) {
    if (this.proxy[scopeId]) {
      const proxy = this.proxy[scopeId];
      delete this.proxy[scopeId];
      proxy.close();
    }
  }

  setupProxy(scopeId, url) {
    const scope = this.scopes[scopeId];
    if (!scope) {
      throw new Error(`Unknown scope ${scopeId}`);
    }

    if (scope.proxy) {
      throw new Error(`A proxy is already setup at ${scopeId}`);
    }

    return new Promise((resolve, reject) => {
      let done = false;
      const proxy = new _ws2.default(url);
      proxy.on('close', () => {
        if (!scope.proxy) {
          // The proxy has already been disassociated, no need to do anything
          return;
        }

        // Remove proxy for this scope
        scope.proxy = null;

        // If the proxy disconnects, close the session as well
        // Since this is a very rare case scenario, may occur when
        // the target group scales down or up
        this.close();
      });

      proxy.on('connect', () => {
        // Proxy connection established, we can resolve the proxy
        if (!done) {
          done = true;
          scope.proxy = proxy;
          resolve(proxy);
        }
      });

      proxy.on('error', () => {
        if (!done) {
          done = true;
          reject(proxy);
        }
      });

      // Forward any message received back to the client
      proxy.on('message', data => {
        this.ws.send(data);
      });
    });
  }

  activate(ws) {
    const parser = (0, _proSocketCommon.createParser)();
    // console.log('parser', parser);
    parser.onScopeRequest = (tracker, scopeId, manifest) => {
      // console.log('onScopeRequest', tracker, scopeId, manifest);
      let scope = this.scopes[scopeId];
      // console.log('scope', scope, scopeId);
      if (!scope) {
        scope = (0, _scoping.findScope)(scopeId);
        if (!scope) {
          return this.send((0, _proSocketCommon.PKT_SCOPE_RESPONSE)(tracker, false, `Unknown scope ${scopeId}`));
        }

        // Store the scope on the session
        this.scopes[scopeId] = scope;

        // Initialize the scope for this session
        scope.init(this);
      }

      if (manifest) {
        return this.send((0, _proSocketCommon.PKT_SCOPE_RESPONSE)(tracker, true, Object.keys(scope.apis)));
      }
      return this.send((0, _proSocketCommon.PKT_SCOPE_RESPONSE)(tracker, true, null));
    };

    parser.onRpcRequest = (tracker, scopeId, api, args) => {
      console.log('onRpcRequest', tracker, scopeId, api, args);
      const apiInstance = { session: this, scope: scopeId };
      const scope = this.scopes[scopeId];
      if (!scope) {
        return this.send((0, _proSocketCommon.PKT_RPC_RESPONSE)(tracker, false, `Unknown api scope ${scopeId}`));
      }

      const fn = scope.apis[api];
      if (!fn) {
        // In case there is proxy available for this scope, then use proxy
        if (scope.proxy) {
          return scope.proxy.send((0, _proSocketCommon.PKT_RPC_REQUEST)(tracker, scopeId, api, args));
        }
        return this.send((0, _proSocketCommon.PKT_RPC_RESPONSE)(tracker, false, `Unknown api ${scopeId}/${api}`));
      }

      return Promise.resolve(fn.apply(apiInstance, args)).then(res => {
        // console.log('propmsie resolove in rpc request', res);
        this.send((0, _proSocketCommon.PKT_RPC_RESPONSE)(tracker, true, res));
      }).catch(err => {
        console.log('error in api', err);
        this.send((0, _proSocketCommon.PKT_RPC_RESPONSE)(tracker, false, err));
      });
    };

    parser.onCall = (scopeId, api, args) => {
      const apiInstance = { session: this, scope: scopeId };
      const scope = this.scopes[scopeId];
      if (!scope) {
        throw new Error(`Unknown scope ${scopeId}`);
      }

      const fn = scope.apis[api];
      if (!fn) {
        if (scope.proxy) {
          return scope.proxy.send((0, _proSocketCommon.PKT_CALL)(scopeId, api, args));
        }
        throw new Error(`Unknown api ${scopeId}/${api}`);
      }

      // Finally execute the method
      return fn.apply(apiInstance, args);
    };

    ws.on('message', data => {
      // console.log('message data', data);
      parser.parse(data);
    });
  }

  close() {
    setImmediate(() => this.ws.close());
  }

  dispatch(action) {
    // console.log('api distapch ', action);
    this.send((0, _proSocketCommon.PKT_ACTION)(action));
  }

  emit(event, data) {
    this.send((0, _proSocketCommon.PKT_EVENT)(event, data));
  }

  scope(name, scoping) {
    if (this.scopes[name]) {
      return this.scopes[name];
    }

    const scope = scoping(this, name);
    this.scopes[name] = scope;
    return scope;
  }

  send(message) {
    if (this.ws.readyState === this.ws.OPEN) {
      this.ws.send(message);
    }
  }

  subscribe(channelId) {
    if (_Channel2.default.subscribe(channelId, this)) {
      this.subscriptions.push(channelId);
    }
  }

  unsubscribe(channelId) {
    if (_Channel2.default.unsubscribe(channelId, this)) {
      const idx = this.subscriptions.indexOf(channelId);
      if (idx >= 0) {
        this.subscriptions.splice(idx);
      }
    }
  }

  // eslint-disable-next-line class-methods-use-this
  channel(id) {
    return new _Channel2.default(id);
  }

  getChannel(channelId) {
    return _Channel2.default.getChannel(channelId, this);
  }

  set(name, value, onClear) {
    // If there is a previous value, clear that first
    if (this.values[name]) {
      this.clear(name);
    }

    this.values[name] = value;
    if (onClear) {
      this.cleanUps[name] = onClear;
    }
  }

  get(name) {
    if (!this.values[name]) {
      throw new Error(`Session doesn't have a value at ${name}`);
    }

    return this.values[name];
  }

  clear(name) {
    if (!this.values[name]) {
      return;
    }

    const onClear = this.cleanUps[name];
    if (onClear) {
      this.onClear();
      delete this.cleanUps[name];
    }

    delete this.values[name];
  }

  addCloseListener(listener) {
    this.closeListeners.push(listener);
  }

  removeCloseListener(listener) {
    const idx = this.closeListeners.indexOf(listener);
    this.closeListeners.splice(idx, 1);
  }
}

exports.default = Session;