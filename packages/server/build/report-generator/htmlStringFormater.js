...
const renderCols = (cols, obj) => {
  return cols.map((col, idx) => `<td style="padding: 8px; text-align: center;">${obj[col] || ''}</td>`);
};
...
exports.default = (reportData, session) => {
  const filePath = _path2.default.join(__dirname, '..', 'public', 'user', `${10000000 + parseInt(session.values.user.id, 10)}`, 'report');
