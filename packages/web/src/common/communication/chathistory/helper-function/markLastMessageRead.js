...
export default (props, state) => {
  ...
  if (unSeenNo > 0) {
    ...
    if (webRtc.chatHistory.type === 'board') {
      apis.addBoardMessageSeenStatus({ userId: account.user.id, bmId: lastMessageId, timeStamp: Date.now(), status: true, broadCastId: `Board-${webRtc.chatHistory.boardDetails.id}` });
