...
const Controller = (props) => {
  const { _callHandler, apis, callType, incomming, answerHandler, webRtc } = props;
  ...
  return (
    <div style={{ cursor: 'pointer' }}>
      ...
      {(incomming || webRtc.liveIncomingCall) && <Button data-tip="End Call" onClick={() => _callHandler(apis, 'audio')} text="" style={{ marginLeft: 5 }} icon="phone" intent="danger" />}
    </div>
  );
};
